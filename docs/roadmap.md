...
agent-ops — being renamed **Olympus**, with the agents taking the deities
whose domains match their functions (Hermes triages, Hephaestus forges,
Themis judges, Argus watches; the full mapping and rename plan live in
[pantheon.md](pantheon.md)) — is heading somewhere specific: an
**open-source, self-hosted, model-agnostic, policy-as-config closed loop**
for autonomous repo maintenance — issue triage → implement → review → revise
→ **production observation feeding back into triage**, plus no-LLM hygiene
guards.
...
3. **Policy-as-config** — one versioned mechanism, per-repo `.olympus.json`
...
- **Design seams early, build adapters late.** Multi-forge adapters are
  ...
  monthly.
- **Dogfood is the credibility proof.** heron as consumer #1 and Olympus's
...
The rename — repo, config filename, env prefixes, default agent names — lands
before anything else ([improvement plan 0.2-0](improvement-plan.md),
[pantheon.md](pantheon.md)). The consumer count is zero today; that window
closes permanently when heron onboards (1.5), so the rename is the one item
whose cost is a function of calendar position, not effort.

**Success:** mechanism code carries zero legacy names; self-CI green;
`examples/consumer/` references `Netis/olympus`.

### 1.1 `olympus init` — one-command onboarding (subsumes `doctor`)
...
`olympus_ref:` (killing the lockstep-by-hand failure), scaffolds a minimal
`.olympus.json`, then runs doctor mode — schema validation, secrets
...
malformed `.olympus.json` fails the PR in guard; raw `gh` outside the seam
is a CI failure.
...
`SELF_DOGFOOD_ENABLED` on this repo so Olympus maintains itself in public.

**Success:** heron runs ≥4 weeks on a pinned tag with zero regression; ≥1
agent-authored PR merged into Olympus by its own loop.
...
### 2.1 The Agora — public demo repo + launch

The **Agora** (`olympus-demo`): a small real service where the entire loop runs in public —
...
Model-agnosticism is a top-3 differentiator, and Olympus is unusual in
...
consumer's own repo — **no central collection**. An `olympus stats` command
...
Codify a named autonomy ladder in `.olympus.json`: **L0** triage-only →
...
*An org runs Olympus on 50 repos and one platform engineer can operate it.*
...
Freeze and document compatibility guarantees: the `.olympus.json` schema
...
### 3.2 Atlas — the fleet CLI (OSS scope)

Stateless, CLI-driven, against a config-file-listed set of repos:

- `atlas bump` — opens version-bump PRs across N repos with migration notes
  inlined. The bump PRs flow through each repo's own review gate — the fleet
  feature is itself loop-shaped.
- `atlas doctor` — 1.1's doctor across the fleet.
- `atlas drift` — diffs each repo's `.olympus.json` against an org baseline
...
Olympus provides only the **gate shape** and each repo supplies the soak
...
| Fleet pin freshness | % of fleet within one minor of latest | >90% with `atlas bump` |
